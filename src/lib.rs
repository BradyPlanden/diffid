...
// Each problem represents a different specification
// Potential problems:
// diffsl + diffsol
// pybamm
// diffrax (potential jax integration problems)
// differentialequations.jl (via juliacall)
...
#[pymethods]
impl Problem {
    fn evaluate(&self, py: Python, x: Vec<f64>) -> PyResult<f64> {
...
fn _core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Builder>()?;
    m.add_class::<SimpleProblem>()?;
    m.add_class::<BuilderFactory>()?;
    m.add_class::<Problem>()?;
    m.add_class::<NelderMead>()?;
    m.add_class::<OptimisationResults>()?;
    Ok(())
}
